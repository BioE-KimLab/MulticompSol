...
print("\n\n\n\nTraining Binary Concat GNN - start at MACHINE TIME",datetime.now())
...
def get_train_test_NFPx2_AltSplits(data, sample_weight, batch_size, fold_number, split_type='shuffle', output_val_col = "DGsolv_constant"):
    ...
    train_all_smiles = list( set(list(train['can_smiles_solvent1']) + list(train['can_smiles_solvent2']) + list(train['can_smiles_solute']) ) )
...
def create_GNN_concat_binary(model_name_in, train_data, valid_data, test_data, 
    ...
    solvent_vector = tf.concat([solvent1_vector, solvent2_vector], -1) 
    solvent_vector = layers.Dense(features_dim, activation='relu')(solvent_vector)
    solvent_vector = layers.Dense(features_dim, activation='relu')(solvent_vector)

    # Combine solvent vector with solute vector
    # prediction is final output
    readout_vector = tf.concat([solvent_vector, global_state_solute], -1)
...
if __name__ == '__main__':
    with tf.device(device):
        ...
        kwargs_concat_binary = {"model_name_in": f"Concat_Binary_{args_argparse.modelname}_{str(args_argparse.split_option).title()}Split_Fold{fold_num_used_NFPx2}",
        ...
        model_path = Path(Path.cwd()/"model_files"/kwargs_concat_binary["model_name_in"])
        ...
        print("\n\nKeyword arguments (train function input)\n\t",kwargs_concat_binary)
        for key, value in kwargs_concat_binary.items():
        ...
        with open("model_files/" + kwargs_concat_binary["model_name_in"] + "/model_params.txt",'w') as f:
        ...
        if run_model is True:
            model_NFPx2, results_NFPx2 = create_GNN_concat_binary(**kwargs_concat_binary)
        else:
