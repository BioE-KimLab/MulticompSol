...
def data_process_all(database, target, data_subset_str):
    ...
    data = db_functions[database](target, 
                                data_subset_str = data_subset_str)
    return data
...
def create_GNN_NFPx2_ShareWeights_ternary(model_name_in, train_data, valid_data, test_data, 
    ...
    edge_states_initial = weight_embedding_edges 
    ...
    # Average or sum node state over solute-solvent1-solvent2-(solvent3) nodes
...
if __name__ == '__main__':
    with tf.device(device):
        ...
        kwargs_GNN_NFPx2 = {"model_name_in": f"Subgraph_Ternary_{args_argparse.modelname}_{str(args_argparse.split_option).title()}Split_Fold{fold_num_used_NFPx2}",
        ...
        if run_model is True:
            model_NFPx2, results_NFPx2 = create_GNN_NFPx2_ShareWeights_ternary(**kwargs_GNN_NFPx2)
        else:
