...
print("\n\n\n\nTraining Binary Subgraph GNN - start at MACHINE TIME",datetime.now())



def data_process_MixSolDBv4_CT_binary(target,
                            data_subset_str = 'b',
                            dummy_smi = 'C',
                            binary_only = True,
                            ):
    ...
    print(f"\n ! Using DB MixSol_v4_CT (CombinedTarget) with target {target}")
    ...
    if target_str not in data.columns:
        print("adding target_str",target_str)
        target_const = tf.constant(list(data[target]))
        data[target_str] = target_const
    
    target_str_const = f"{target}_constant"
    data[target_str_const] = data[target_str_const].astype(float)
    ...
    print("\tBefore:\t", data.shape)
    data = data.dropna(subset=target_str_const)
    print("\tAfter:\t", data.shape)
...
def data_process_all(database, target,
                    data_subset_str_val='b', dummy_smi_val='C'):
    db_options = {
    ...
    db_functions = {
                    "MixSol_v4_CT": data_process_MixSolDBv4_CT_binary,
                  }

    data = db_functions[database](target, data_subset_str=data_subset_str_val, dummy_smi=dummy_smi_val, binary_only=True)
...
def get_train_test_NFPx2_AltSplits(data, sample_weight, batch_size, fold_number, split_type='shuffle', output_val_col = "DGsolv_constant"):
    print(f"Using output val col of '{output_val_col}'.")
    X_data = data[['can_smiles_solute', 'can_smiles_solvent1',
                       'can_smiles_solvent2', 'mol_frac_solvent1',
                       'mol_frac_solvent2', 'T_K', ]]
    ...
    train_all_smiles = list( set(list(train['can_smiles_solvent1']) + list(train['can_smiles_solvent2']) + list(train['can_smiles_solute']) ) ) #! CHANGED: 
    ...
    train_data = tf.data.Dataset.from_generator(
        lambda: create_tf_dataset_NFPx2(train, preprocessor, sample_weight, True, output_val_col = output_val_col), output_signature=output_signature)\
    ...
    valid_data = tf.data.Dataset.from_generator(
        lambda: create_tf_dataset_NFPx2(valid, preprocessor, sample_weight, False, output_val_col = output_val_col), output_signature=output_signature)\
        .cache()\
        .padded_batch(batch_size=batch_size)\
        .prefetch(tf.data.experimental.AUTOTUNE)

    test_data = tf.data.Dataset.from_generator(
        lambda: create_tf_dataset_NFPx2(test, preprocessor, sample_weight, False, output_val_col = output_val_col), output_signature=output_signature)\
        .cache()\
        .padded_batch(batch_size=batch_size)\
        .prefetch(tf.data.experimental.AUTOTUNE)
    
    train_data_final = tf.data.Dataset.from_generator(
        lambda: create_tf_dataset_NFPx2(train, preprocessor, sample_weight, False, output_val_col = output_val_col), output_signature=output_signature)\
        .cache()\
        .padded_batch(batch_size=batch_size)\
        .prefetch(tf.data.experimental.AUTOTUNE)
...
def create_GNN_NFPx2(model_name_in, train_data, valid_data, test_data, 
    ...
    node_states_initial = tf.stack([global_state_solute, 
                                           global_state_solvent1, 
                                           global_state_solvent2], axis=1)
    print("NODE STATES INITIAL",node_states_initial)
    stoich_vec_6edge = tf.stack([ratio_Input_solvent1 + ratio_Input_solvent2,
                                ratio_Input_solvent1 + ratio_Input_solvent2,
    ...
    if do_stoich_multiply == 'before_dense':
        stoich_multiply_tensor = tf.stack([ratio_Input_solvent1 + ratio_Input_solvent2, ratio_Input_solvent1, ratio_Input_solvent2], axis=1)
    ...
    print(f"Fold number is: {fold_number} - previously defaulted to 0, check to match data split")
...
if __name__ == '__main__':
    with tf.device(device):
        ...
        parser.add_argument('--db', type=str, default='MixSol_v4_CT', help='Database of binary solubilities to be used (default=MixSol_v1). Options: ["MixSol_v1" (logS, DGsolv), "MixSol_v3" (logS, DGsolv), "ComboDB_v1" (logS)]')
        ...
        data_in = data_process_all(database = args_argparse.db, target = args_argparse.target,
                                    data_subset_str_val='b', dummy_smi_val='C')
        ...
        if split_type == 'shuffle':
            ...
            preprocessor_NFPx2,out_sig_NFPx2,datasets_NFPx2,dataframes_NFPx2 = get_train_test_NFPx2_AltSplits(data=data_in, sample_weight=sample_weight_NFPx2, 
        ...
        kwargs_GNN_NFPx2 = {"model_name_in": f"Subgraph_Binary_{args_argparse.modelname}_{str(args_argparse.split_option).title()}Split_Fold{fold_num_used_NFPx2}",
        ...
        model_path = Path(Path.cwd()/"model_files"/kwargs_GNN_NFPx2["model_name_in"])
        ...
        for key, value in kwargs_GNN_NFPx2.items():
        ...
        with open("model_files/" + kwargs_GNN_NFPx2["model_name_in"] + "/model_params.txt",'w') as f:
        ...
        if run_model is True:
            model_NFPx2, results_NFPx2 = create_GNN_NFPx2(**kwargs_GNN_NFPx2)
        else:
